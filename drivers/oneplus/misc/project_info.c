...
void save_dump_reason_to_smem(char *info, char *function_name)
{
	...
	if (IS_ERR_OR_NULL(dp_info))
		...
		if (info) {
			strl = strlen(info) + 1;
			strl = strl < DUMP_REASON_SIZE ? strl :
							 DUMP_REASON_SIZE;
			if ((strlen(dp_info->dump_reason) + strl) <
			    DUMP_REASON_SIZE)
		...
		if (function_name) {
			...
			strl1 = strl1 < DUMP_REASON_SIZE ? strl1 :
							   DUMP_REASON_SIZE;
			if ((strlen(dp_info->dump_reason) + strl1 + 3) <
			    DUMP_REASON_SIZE) {
				strncat(dp_info->dump_reason, "\r\n", 2);
				strncat(dp_info->dump_reason, function_name,
					strl1);
			}
		} else
		...
		if ((strcmp(caller_function_name, "panic") == 0)) {
			...
			if (regs) {
				...
				length = length < DUMP_REASON_SIZE ?
						 length :
						 DUMP_REASON_SIZE;
				if ((strlen(dp_info->dump_reason) + length +
				     12) < DUMP_REASON_SIZE) {
					strncat(dp_info->dump_reason, "\r\n",
						2);
					strncpy(buf, "PC at:", 7);
					strncat(dp_info->dump_reason, buf, 7);
					strncat(dp_info->dump_reason, buf1,
						length);
					strncat(dp_info->dump_reason, "\r\n",
						2);
				}
	...
	pr_debug(
		...
		__func__, dp_info->dump_reason, strl, function_name, strl1);
...
struct cpu_list cpu_list_msm[] = {
	...
	{ 400, "SM7250 " }, { 459, "SM7225 " }, { 434, "SM6350 " },
	{ 417, "SM4250 " }, { 0, "Unknown" },
};
...
static int op_aboard_read_gpio(void)
{
	...
	a_board_val = gpio0;
	snprintf(Aboard_version, sizeof(Aboard_version), "%d %s", a_board_val,
		 a_board_version_string_arry_gpio[a_board_val].name);
...
static int oem_aboard_probe(struct platform_device *pdev)
{
	...
	data->pinctrl_state_sleep =
		pinctrl_lookup_state(data->pinctrl, "oem_aboard_sleep");
