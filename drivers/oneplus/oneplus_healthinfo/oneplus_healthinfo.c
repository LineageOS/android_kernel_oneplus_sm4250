...
static char *sched_list[OHM_TYPE_TOTAL] = {
	/* SCHED_STATS 0 -11 */
	"iowait", "sched_latency", "fsync", "emmcio", "downread", "downwrite",
	/*2020-06-22 ，OSP-5970 , monitor cpu info **/
	"dstate", "sched_default_05", "sched_default_06", "sched_default_07",
	"sched_default_10", "sched_default_11",
	/* OTHER_TYPE 12 - */
	"cur_cpu_load", "memory_monitor", "io_panic", "rt_info",
	"preempt_latency"
};
...
void ohm_action_trig(int type)
{
	...
	if (OHM_MEM_MON == type || OHM_SCHED_FSYNC == type) {
		...
		if (likely(kworker))
			kthread_queue_delayed_work(kworker, &ohm_work,
						   msecs_to_jiffies(1));
		rcu_read_unlock();
	}
}
...
void ohm_action_trig_level(int type, bool highlevel)
{
	...
	if (OHM_MEM_MON == type || OHM_SCHED_FSYNC == type ||
		...
		if (!time_after(jiffies,
				last_trig +
					msecs_to_jiffies(TRIG_MIN_INTERVAL)))
			return;
		if (atomic_cmpxchg(&ohm_scheduled, 0, 1) != 0)
			return;
		ohm_debug_deferred("%s trig action\n", sched_list[type]);
		sprintf(ohm_detect_env[1], "OHMTYPE=%s", sched_list[type]);
		sprintf(ohm_detect_env[2], "OHMLEVEL=%s",
			highlevel ? "HIGH" : "LOW");
		ohm_detect_env[MAX_OHMEVENT_PARAM - 1] = NULL;
		rcu_read_lock();
		kworker = rcu_dereference(ohm_kworker);
	...
	if (task_is_fg(task)) {
		...
		if (sched_type == OHM_SCHED_IOWAIT) {
			if (time_after(jiffies,
				...
				iowait_summ_start =
					jiffies - msecs_to_jiffies(delta_ms);
...
void ohm_overload_record(struct rq *rq, u64 delta)
{
	...
	if (sched_stat != NULL) {
		...
		ohm_sched_stat_record_common(sched_stat, &sched_stat->all,
					     delta);
	}
	return;
}
static inline void
ohm_preempt_stat_record_common(struct cpu_preempt_stat *sched_stat,
			       struct sched_stat_common *stat_common,
			       u64 delta_ms)
{
	stat_common->total_ms += delta_ms;
...
void ohm_preempt_record(u64 delta, int cpu)
{
	...
	ohm_preempt_stat_record_common(preempt_stat,
				       &(preempt_stat->preempt_common), delta);
	return;
}
...
void update_stuck_trace_info(struct task_struct *tsk, int trace_type,
			     unsigned int cpu, u64 delta)
{
	...
	if (!ltt_cpu_nr) {
		...
		printk("fuyou_update_stuck_trace_info ltt_cpu_nr = %u",
		       ltt_cpu_nr);
	}
...
void ohm_iolatency_record(struct request *req, unsigned int nr_bytes, int fg,
			  u64 delta_us)
{
	...
	if (delta_ms >= oneplus_io_para.high_thresh_ms) {
		...
		if (oneplus_io_para.logon) {
			ohm_debug(
				"[io latency / %s] high_cnt, delay = %llu ms\n",
				(fg ? "fg" : "bg"), delta_ms);
		}
		if (fg) {
	...
	if (fg) {
		if ((req_op(req) != REQ_OP_DISCARD) &&
			...
			if (req_op(req) == REQ_OP_WRITE ||
			    req_op(req) == REQ_OP_WRITE_SAME) {
				oneplus_io_para.iosize_write_count_fg++;
				oneplus_io_para.iosize_write_us_fg += delta_us;
				if (rq_is_sync(req)) {
					if (delta_ms > 500) {
						oneplus_io_para
							.iosize_500ms_syncwrite_count_fg++;
					} else if (delta_ms > 200) {
						oneplus_io_para
							.iosize_200ms_syncwrite_count_fg++;
					}
				} else {
					if (delta_ms > 500) {
						oneplus_io_para
							.iosize_500ms_asyncwrite_count_fg++;
					} else if (delta_ms > 200) {
						oneplus_io_para
							.iosize_200ms_asyncwrite_count_fg++;
					}
				}
			} else {
				...
				if (delta_ms > 500) {
					oneplus_io_para
						.iosize_500ms_read_count_fg++;
				} else if (delta_ms > 200) {
					oneplus_io_para
						.iosize_200ms_read_count_fg++;
				}
			}
		}
	} else {
		if ((req_op(req) != REQ_OP_DISCARD) &&
			...
			if (req_op(req) == REQ_OP_WRITE ||
				...
				if (rq_is_sync(req)) {
					if (delta_ms > 2000) {
						oneplus_io_para
							.iosize_2s_syncwrite_count_bg++;
						if (oneplus_io_para.trig)
							ohm_action_trig(
								OHM_SCHED_EMMCIO);
					} else if (delta_ms > 500) {
						oneplus_io_para
							.iosize_500ms_syncwrite_count_bg++;
					} else if (delta_ms > 200) {
						oneplus_io_para
							.iosize_200ms_syncwrite_count_bg++;
					}
				} else {
					if (delta_ms > 2000) {
						oneplus_io_para
							.iosize_2s_asyncwrite_count_bg++;
						if (oneplus_io_para.trig)
							ohm_action_trig(
								OHM_SCHED_EMMCIO);
					} else if (delta_ms > 500) {
						oneplus_io_para
							.iosize_500ms_asyncwrite_count_bg++;
					} else if (delta_ms > 200) {
						oneplus_io_para
							.iosize_200ms_asyncwrite_count_bg++;
					}
				}
			} else {
				...
				if (delta_ms > 2000) {
					oneplus_io_para
						.iosize_2s_read_count_bg++;
					if (oneplus_io_para.trig)
						ohm_action_trig(
							OHM_SCHED_EMMCIO);
				} else if (delta_ms > 500) {
					oneplus_io_para
						.iosize_500ms_read_count_bg++;
				} else if (delta_ms > 200) {
					oneplus_io_para
						.iosize_200ms_read_count_bg++;
				}
			}
		}
	}
...
static inline void ohm_rt_para_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		rt_para[i].lt_info = (struct longest_task_info *)kzalloc(
			sizeof(struct longest_task_info), GFP_ATOMIC);
	}
...
void ohm_trig_init(void)
{
	...
	for (i = 0; i < OHM_SCHED_TOTAL; i++) {
		oneplus_sched_para[i].trig =
			(ohm_trig_list & BIT(i)) ? true : false;
		if (i == OHM_SCHED_EMMCIO)
			oneplus_io_para.trig =
				(ohm_trig_list & BIT(i)) ? true : false;
	}
	return;
}
...
void ohm_logon_init(void)
{
	...
	for (i = 0; i < OHM_SCHED_TOTAL; i++) {
		oneplus_sched_para[i].logon =
			(ohm_logon_list & BIT(i)) ? true : false;
		if (i == OHM_SCHED_EMMCIO)
			oneplus_io_para.logon =
				(ohm_logon_list & BIT(i)) ? true : false;
	}
	return;
}
...
void ohm_ctrl_init(void)
{
	...
	for (i = 0; i < OHM_SCHED_TOTAL; i++) {
		oneplus_sched_para[i].ctrl =
			(ohm_ctrl_list & BIT(i)) ? true : false;
		if (i == OHM_SCHED_EMMCIO)
			oneplus_io_para.ctrl =
				(ohm_ctrl_list & BIT(i)) ? true : false;
	}
...
void ohm_para_init(void)
{
	int i;
	for (i = 0; i < OHM_SCHED_TOTAL; i++) {
		memset(&oneplus_sched_para[i], 0,
		       sizeof(struct sched_stat_para));
	...
	oneplus_sched_para[OHM_SCHED_EMMCIO].low_thresh_ms =
		LOW_THRESH_MS_DEFAULT;
	oneplus_sched_para[OHM_SCHED_EMMCIO].high_thresh_ms =
		HIGH_THRESH_MS_DEFAULT;
			   ...
			   loff_t *off)
{
	...
	len = sprintf(
		...
		oneplus_io_para.low_cnt,
		//high thresh parameter
		oneplus_io_para.high_thresh_ms, oneplus_io_para.high_cnt,
		//total parameter
		(oneplus_io_para.total_us / 1000),
		(oneplus_io_para.emmc_total_us / 1000),
		(oneplus_io_para.total_us - oneplus_io_para.emmc_total_us) /
			1000,
		oneplus_io_para.total_cnt,
		//fg latency parameter
		oneplus_io_para.fg_low_cnt, oneplus_io_para.fg_high_cnt,
		oneplus_io_para.fg_total_ms, oneplus_io_para.fg_total_cnt,
		oneplus_io_para.fg_max_delta_ms, oneplus_io_para.delta_ms,
		//fg
		oneplus_io_para.iosize_write_count_fg,
...
/******  irqs latency  ******/
#define COMMON_STRING_FORMAT(BUF, MODULE, SCHED_STAT, NUM)                     \
	sprintf(BUF,                                                           \
		...
		NUM, SCHED_STAT->max, NUM, SCHED_STAT->high_cnt, NUM,          \
		SCHED_STAT->low_cnt, NUM, SCHED_STAT->total, NUM,              \
		SCHED_STAT->total_cnt)

static ssize_t irq_latency_read(struct file *filp, char __user *buff,
				size_t count, loff_t *off)
{
	int len = 0, i;
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		len += COMMON_STRING_FORMAT(page + len, irq_latency, sched_stat,
					    i);
	}
...
/****** Preempt state *****/
#define PREEMPT_STRING_FORMAT(BUF, SCHED_STAT, NUM)                            \
	sprintf(BUF,                                                           \
		...
		NUM, SCHED_STAT.max_ms, NUM, SCHED_STAT.high_cnt, NUM,         \
		SCHED_STAT.low_cnt, NUM, SCHED_STAT.total_ms, NUM,             \
		SCHED_STAT.total_cnt)

static ssize_t preempt_latency_read(struct file *filp, char __user *buff,
				    size_t count, loff_t *off)
{
	int len = 0, j;
	char *page = kzalloc(2048, GFP_KERNEL);
	...
	len = sprintf(page,
		      "Show preemp-latency stat on per-cpu \n"
		      "\nCpu count:%d\n",
		      NR_CPUS);
	for (j = 0; j < NR_CPUS; j++) {
		len += sprintf(page + len, "cpu:%d\n", j);
		len += PREEMPT_STRING_FORMAT(page + len,
					     preempt_para[j].preempt_common, j);
	}
...
void rt_thresh_times_record(struct task_struct *p, unsigned int cpu)
{
	...
	if (rt_para[cpu].lt_info->max_exec_ns < exec_runtime_ns &&
	    exec_runtime_ns > 0 && exec_runtime_ns < MAX_RT_EXEC) {
		rt_para[cpu].lt_info->max_exec_ns = exec_runtime_ns;
		strcpy(rt_para[cpu].lt_info->comment, p->comm);
	}
	if (exec_runtime_ns < MAX_RT_EXEC &&
	    (exec_runtime_ns) > DEFAULT_RT_HT) {
		rt_para[cpu].thresh_cnt[0]++;
	} else if (DEFAULT_RT_HT > exec_runtime_ns &&
		   exec_runtime_ns > DEFAULT_RT_LT) {
...
static ssize_t cpu_rtime_read(struct file *filp, char __user *buff,
			      size_t count, loff_t *off)
{
	int len = 0, i, j;
	char *page = kzalloc(2048, GFP_KERNEL);
	...
	len = sprintf(page,
		      "Collect information about rt processes on per cpu \n"
		      "\nCpu count:%d\n"
		      "High_thresh_ns:%d\n"
		      "Low_thresh_ns:%d\n",
	...
	for (i = 0; i < NR_CPUS; i++) {
		len += sprintf(page + len,
			       ...
			       i, i, rt_para[i].each_cpu_rt_total, i,
			       rt_para[i].each_cpu_rt, i,
			       rt_para[i].lt_info->comment, i,
			       rt_para[i].lt_info->max_exec_ns);
		for (j = 0; j < 3; j++) {
			if (j != 0)
				len += sprintf(page + len,
					       "cpu%d_%s_thresh_count: %llu\n",
					       i, thresh_list[j],
					       rt_para[i].thresh_cnt[j]);
			else
				len += sprintf(page + len,
					       "cpu%d_%s_thresh_count: %d\n", i,
					       thresh_list[j],
					       rt_para[i].thresh_cnt[j]);
		}
	}
...
#define OVERLOAD_STRING_FORMAT(BUF, SCHED_STAT, NUM)                           \
	sprintf(BUF,                                                           \
		...
		NUM, SCHED_STAT->high_thresh_ms, NUM, SCHED_STAT->all.max_ms,  \
		NUM, SCHED_STAT->all.high_cnt, NUM, SCHED_STAT->all.low_cnt,   \
		NUM, SCHED_STAT->all.total_ms, NUM, SCHED_STAT->all.total_cnt)

static ssize_t overload_read(struct file *filp, char __user *buff, size_t count,
			     loff_t *off)
{
	...
	for_each_sched_cluster(cluster)
	{
		...
		len += OVERLOAD_STRING_FORMAT(page + len, cluster->overload,
					      cluster->id);
	}
	return sched_data_to_user(buff, count, off, page, len);
}
			       ...
			       size_t count, loff_t *off)
{
	...
	len = sprintf(page,
		      ...
		      allocwait_para.total_alloc_wait_max_ms,
		      allocwait_para.total_alloc_wait_max_order,
		      allocwait_para.fg_alloc_wait_max_ms,
	...
	do_each_thread(group, p)
	{
		...
		if (p->tli[!index].tli_overload_flag &
			...
			seq_printf(
				s,
				"%-10d\t%-10d\t%-16s\t%-16llu\t%-8s\t%-16llu\n",
				p->pid, p->tgid, p->comm, spead, "read",
				task_index);
		}
	}
...
static int runtime_overload_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "%-10s\t%-10s\t%-16s\t%-16s\t%-6s\t%-6s\t%-16s\n", "TID",
		   "TGID", "COMM", "runtime", "FG/BG", "RT", "task_index");
	do_each_thread(group, p)
	{
		if (window_index != (p->tli[!index].task_sample_index + 1))
			continue;
		...
		if (p->tli[!index].tli_overload_flag &
			...
			seq_printf(
				s,
				"%-10d\t%-10d\t%-16s\t%-16llu\t%-6s\t%-6s\t%-16llu\n",
				p->pid, p->tgid, p->comm, runtime, "FG",
				rt ? "YES" : "NO", task_index);
		}
		if (p->tli[!index].tli_overload_flag &
			...
			seq_printf(
				s,
				"%-10d\t%-10d\t%-16s\t%-16llu\t%-6s\t%-6s\t%-16llu\n",
				p->pid, p->tgid, p->comm, runtime, "BG",
				rt ? "YES" : "NO", task_index);
		}
	}
	while_each_thread(group, p);
...
static void adjust_window(struct timer_list *unused)
{
	...
	mod_timer(&task_load_info_timer,
		  jiffies + ohm_sample_time * HZ); /* 5s */
}
#endif
...
static int __init oneplus_healthinfo_init(void)
{
	...
	/******  ctrl  *****/
	pentry = proc_create("para_update", S_IRUGO | S_IWUGO,
			     oneplus_healthinfo, &proc_para_fops);
	if (!pentry) {
	...
	/******  Stat  ******/
	pentry = proc_create("fsync_wait", S_IRUGO, oneplus_healthinfo,
	...
	pentry = proc_create("cpu_loading", S_IRUGO, oneplus_healthinfo,
			     &proc_cpu_load_fops);
	if (!pentry) {
	...
	pentry = proc_create("iowait", S_IRUGO, oneplus_healthinfo,
			     &proc_iowait_fops);
	if (!pentry) {
	...
	pentry = proc_create("sched_latency", S_IRUGO, oneplus_healthinfo,
	...
	pentry = proc_create("emcdrv_iowait", S_IRUGO, oneplus_healthinfo,
			     &proc_emmcio_fops);
	if (!pentry) {
	...
	pentry = proc_create("iowait_hung", S_IRUGO, oneplus_healthinfo,
	...
	/*2020-06-22 ，OSP-5970 , monitor cpu info **/
	pentry = proc_create("dstate", S_IRUGO, oneplus_healthinfo,
			     &proc_dstate_fops);
	if (!pentry) {
	...
	pentry = proc_create("overload", S_IRUGO, oneplus_healthinfo,
			     &proc_overload_fops);
	if (!pentry) {
		ohm_err("create overload proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
	pentry = proc_create("cpu_rt_info", S_IRUGO, oneplus_healthinfo,
			     &proc_cpu_rtime_stat_fops);
	if (!pentry) {
		ohm_err("create cpu_rtime_read proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
	pentry = proc_create("preempt_latency", S_IRUGO, oneplus_healthinfo,
			     &proc_preempt_latency_fops);
	if (!pentry) {
		ohm_err("create preempt latency proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
	pentry = proc_create("irq_latency", S_IRUGO, oneplus_healthinfo,
			     &proc_irq_latency_fops);
	if (!pentry) {
...
#ifdef CONFIG_ONEPLUS_MEM_MONITOR
	pentry = proc_create("alloc_wait", S_IRUGO, oneplus_healthinfo,
	...
	pentry = proc_create("rw_overload", S_IRUGO, oneplus_healthinfo,
	...
	pentry = proc_create("runtime_overload", S_IRUGO, oneplus_healthinfo,
			     &proc_runtime_overload_fops);
	if (!pentry) {
...
module_param_named(ohm_iowait_l_ms,
		   oneplus_sched_para[OHM_SCHED_IOWAIT].low_thresh_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_iowait_h_ms,
		   oneplus_sched_para[OHM_SCHED_IOWAIT].high_thresh_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_schedlatency_l_ms,
		   ohm_thresh_para[OHM_SCHED_SCHEDLATENCY].l_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_schedlatency_h_ms,
		   ohm_thresh_para[OHM_SCHED_SCHEDLATENCY].h_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_fsync_l_ms, ohm_thresh_para[OHM_SCHED_FSYNC].l_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_fsync_h_ms, ohm_thresh_para[OHM_SCHED_FSYNC].h_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_emmcio_l_ms, ohm_thresh_para[OHM_SCHED_EMMCIO].l_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_emmcio_h_ms, ohm_thresh_para[OHM_SCHED_EMMCIO].h_ms, int,
		   S_IRUGO | S_IWUSR);
module_param_named(iowait_summ_period, iowait_summ_period, int,
		   S_IRUGO | S_IWUSR);
module_param_named(iowait_summ_thresh, iowait_summ_thresh, int,
		   S_IRUGO | S_IWUSR);
#ifdef CONFIG_ONEPLUS_TASKLOAD_INFO
module_param_named(ohm_write_thresh, ohm_write_thresh, ullong,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_read_thresh, ohm_read_thresh, ullong, S_IRUGO | S_IWUSR);
module_param_named(ohm_runtime_thresh_fg, ohm_runtime_thresh_fg, ullong,
		   S_IRUGO | S_IWUSR);
module_param_named(ohm_runtime_thresh_bg, ohm_runtime_thresh_bg, ullong,
		   S_IRUGO | S_IWUSR);
